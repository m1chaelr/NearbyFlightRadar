...
def getData():
    """
    An authenticated endpoint to get flight data for TRMNL Polling. 
    For now, it fetches live data on each call rather than relying on a cached JSON file.
    """
    provided_key = request.headers.get("X-API-KEY")
    ...
    try:
        # TEMPORARY: Directly call getFlightRadar instead of using the cache file
        flight_data = getFlightRadar(deploy_mode)
        return jsonify(flight_data)
